...
void ramsete::follow(std::vector<std::vector<double>> path, double initlinvel,
                     double initangvel, double finalheading, double settletime,
  ...
  // loop over entire vector ecept for last point (due to redundancy in path
  // generation)
  for (int i = 0; i <= path.size() - 1; ++i) {
    ...
    while (true) {
      ...
      double errorx = (targetx - currentx) * cos(currentheading) +
                      (targety - currenty) * sin(currentheading);
      double errory = -sin(currentheading) * (targetx - currentx) +
                      cos(currentheading) * (targety - currenty);
      double errorheading = targetheading - currentheading;
      // exit within tolerance
      if ((fabs(errorx) < .2 && fabs(errory) < .2 && fabs(errorheading) < .5) ||
          time.isDone() == true) {
        break;
      }
      // compute gainvalues
      double b = 1;    // roughly proportional
      double zeta = 1; // dampening term
      double k =
          2 * zeta *
          sqrt(pow(targetangvel, 2) + (b * pow(targetlinvel, 2))); // gainvalue
      // compute output velocities
      double linvel = targetlinvel * cos(errorheading) + k * errorx;
      double angvel;
      if (errorheading != 0) {
        angvel =
            targetangvel + k * errorheading +
            ((b * targetlinvel * sin(errorheading) * errory) / errorheading);
      } else {
        angvel = 0;
      }
