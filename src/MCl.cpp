...
std::vector<double> monte::carlo () {
    ...
    double thetaoff = pose.theta - prevpose.theta;
    double noise = 5;
    //weighting vars
    double totalweights = 0;
    double highestweight = 0;
    double highestweightindex = 0;
    //update particles
    ...
    if (totalweights > 0) {
        ...
        // Resampling based on normalized weights
        std::vector<double> cumulative_weights(particles.size(), 0.0);
        cumulative_weights[0] = particles[0][3];  // Normalize first weight
        for (int i = 1; i < particles.size(); ++i) {
            cumulative_weights[i] = cumulative_weights[i - 1] + particles[i][3];
        }

        // Resample particles
        for (int i = 0; i < particles.size(); ++i) {
            double random_value = randomDoubleInRange(0.0, 1.0);
