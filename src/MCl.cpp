...
void monte::carlo() {
  ...
  for (int i = 0; i < particles.size(); ++i) {
    ...
    particles[i][0] = (particles[i][0] >= map[0] && particles[i][0] <= map[1]) ? (particles[i][0] + randomDoubleInRange(xoff - noise, xoff + noise))
                                                                               : (particles[i][0] > 0 ? map[1] : map[0]);
    particles[i][1] = (particles[i][1] >= map[0] && particles[i][1] <= map[1]) ? (particles[i][1] + randomDoubleInRange(yoff - noise, yoff + noise))
                                                                               : (particles[i][1] > 0 ? map[1] : map[0]);
...
double monte::randomDoubleInRange(double min, double max) { return min + (std::rand() / (RAND_MAX + 1.0)) * (max - min); }
