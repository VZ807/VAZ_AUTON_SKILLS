...
public:
  ...
  struct Rectangle {
    ...
    Rectangle(point TL, point TR, point BL, point BR) : TL(TL), TR(TR), BL(BL), BR(BR), top(TL, TR), bottom(BL, BR), left(TL, BL), right(TR, BR) {}
  };
  /**
  ...
  struct distsensconfig {
    ...
    float distsensoffy;
    float angle;
    distsensconfig(float distsensoffx, float distsensoffy, float angle) : distsensoffx(distsensoffx), distsensoffy(distsensoffy), angle(angle) {}
  };
};
#endif // MCL_HPP